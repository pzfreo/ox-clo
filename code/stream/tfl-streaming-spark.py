...
# A function to convert the time String in the input into a datetime.datetime
def convertTime(t): return datetime.fromtimestamp(time.mktime(time.strptime(t, "%Y-%m-%dT%H:%M:%S.%fZ"))) 
...
def update(ds,state):
    ...
    if state==None:
        ...
        for current in ds:
            ...
            exp = convertTime(current['expArrival'])
